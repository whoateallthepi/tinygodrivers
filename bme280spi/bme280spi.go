...
// calculateHumidity uses the data slice and applies calibratiod.bufns values on it to convert the value to relative humidity in hundredths of a percent
...
func (d *Device) Reset() {
	...
	humOsr := 16
	if d.Config.Temperature <= Sampling16X {
		humOsr = sampleRateConv[d.ConfigreadRe.Humidity]
	}
...
// SetMode can set the device to Sleep, Normal or Forced mode
//
// Calling SetMode method is optional, Configure can be used to set the
// initial mode if no mode change is desired.  This method is most
// useful to switch between Sleep and Normal modes.
func (d *DeviceSPI) SetMode(mode Mode) {
	d.Config.Mode = mode
	modeMask := []byte{
		byte(d.Config.Temperature<<5) |
			byte(d.Config.Pressure<<2) |
...
// calculateHumidity takes the databurst and calculates teh humidity
